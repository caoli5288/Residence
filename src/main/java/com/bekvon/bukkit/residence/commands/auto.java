...
public class auto implements cmd {
    ...
    public boolean perform(Residence plugin, String[] args, boolean resadmin, Command command, CommandSender sender) {
		...
		if (lenght > 0) {
			if (lenght < group.getMaxX() + b.getMaxWeight() && lenght > X)
				X = lenght;
			if (lenght < group.getMaxZ() + b.getMaxWeight() && lenght > Z)
				Z = lenght;
		}
    ...
    private static void resize(Residence plugin, Player player, CuboidArea cuboid) {
		...
		while (true) {
			...
			if (c.getXSize() >= group.getMaxX() + b.getMaxWeight() - group.getMinX()) {
				locked.add(dir);
				dir = dir.getNext();
				skipped++;
				continue;
			...
			if (c.getYSize() >= group.getMaxY() + b.getMaxHeight() - group.getMinY()) {
				locked.add(dir);
				dir = dir.getNext();
				skipped++;
				continue;
			...
			if (c.getZSize() >= group.getMaxZ() + b.getMaxWeight() - group.getMinZ()) {
				locked.add(dir);
				dir = dir.getNext();
				skipped++;
				continue;
